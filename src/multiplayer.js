...
export async function initMultiplayer(sceneRef, boat, cameraRef) {
  ...
  let DbConnection;
  try {
    // import.meta.glob tolerates the bindings being absent at build time —
    // they are generated by the dev-server SpacetimeDB plugin and gitignored,
    // so a plain import() breaks `vite build` on a fresh checkout / CI.
    const bindingModules = import.meta.glob('./module_bindings/index.ts');
    const loadBindings = bindingModules['./module_bindings/index.ts'];
    const bindings = loadBindings ? await loadBindings() : null;
    DbConnection = bindings?.DbConnection;
    if (!DbConnection) {
      console.warn('Multiplayer: SpacetimeDB bindings not found — running single-player.');
      return;
    }
  } catch (err) {
    console.warn('Multiplayer: failed to load SpacetimeDB bindings.', err);
    return;
  }
...
// The preset API below is the client half of reducers that already exist in
// the server module; no UI calls it yet (planned for the design-mode epic,
// ticket #8 "Object library, world placement & serialization").
